...
Error_t CFft::overrideWindow( const float *pfNewWindow )
{
    ...
    CUtil::copyBuff(m_pfWindowBuff, pfNewWindow, m_iDataLength);
...
Error_t CFft::getWindow( float *pfWindow ) const
{
    ...
    CUtil::copyBuff(pfWindow, m_pfWindowBuff, m_iDataLength);
...
Error_t CFft::doFft( complex_t *pfSpectrum, const float *pfInput )
{
    ...
    CUtil::copyBuff(m_pfProcessBuff, pfInput, m_iDataLength);
    CUtil::setZero(&m_pfProcessBuff[m_iDataLength], m_iFftLength-m_iDataLength);
    ...
    CUtil::copyBuff(pfSpectrum, m_pfProcessBuff, m_iFftLength);
...
Error_t CFft::doInvFft( float *pfOutput, const complex_t *pfSpectrum )
{
    ...
    // copy data to internal buffer
    CUtil::copyBuff(m_pfProcessBuff, pfSpectrum, m_iFftLength);
